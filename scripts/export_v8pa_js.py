#!/usr/bin/env python3
"""Export V8pa as Piper lgbm_evaluate JS (BCI + path A, no no_underage merge)."""
...
def build_script(feats, trees, meta):
    ...
    # NOTE: V8pa is for d2911d10bb (no no_underage). buildVec routes underage_/adult_ only.
    return f"""// lgbm_evaluate — Piper node (V8pa)
...
def main():
    feats = json.loads((DATA / 'lgbm_v8pa_features.json').read_text())
    model_str = (DATA / 'lgbm_underage_v8pa.txt').read_text()
    meta = json.loads((DATA / 'lgbm_v8pa_meta.json').read_text())
    trees = parse_trees(model_str)
    js = build_script(feats, trees, meta)
    out = DATA / 'lgbm_evaluate_v8pa.js'
    out.write_text(js)
    print(f'V8pa: features={len(feats)}, trees={len(trees)} → {out} ({len(js)//1024} KB)', flush=True)
