#!/usr/bin/env python3
"""Export V11 (BCI + hard-neg) as Piper lgbm_evaluate JS.

Differences from V10pa JS:
  - buildVec also computes 4 BCI aggregates (_child_body, _child_context,
    _child_interaction, _body_vs_context) on-the-fly using BODY/CONTEXT/INTERACTION
    label sets baked into the JS.
  - getAdjustedScores still has multiplier disabled (Path A).
"""
...
def build_script(feats, trees, meta):
    ...
    return f"""// lgbm_evaluate — Piper node (V11)
// Path A + BCI feature split + hard-neg mining x20
...
function buildVec(labelsFlat) {{
  ...
  // Step 1: collect raw underage scores (with :x stripped) for BCI aggregates
  ...
  for (const key in labelsFlat) {{
    ...
    if (key.startsWith('adult__')) {{
      ...
      if (fname in featIdx) vec[featIdx[fname]] = val;
      // keep raw value indexed by stripped name for BCI aggregation
...
def main():
    feats = json.loads((DATA / 'lgbm_v11_features.json').read_text())
    model_str = (DATA / 'lgbm_underage_v11.txt').read_text()
    meta = json.loads((DATA / 'lgbm_v11_meta.json').read_text())
    trees = parse_trees(model_str)
    js = build_script(feats, trees, meta)
    out = DATA / 'lgbm_evaluate_v11.js'
    out.write_text(js)
    print(f'V11: features={len(feats)}, trees={len(trees)} → {out} ({len(js)//1024} KB)', flush=True)
